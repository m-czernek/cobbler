...
class CobblerCheck:
    ...
    def check_name(self, status):
        ...
        if self.settings.server == "127.0.0.1":
            status.append("The 'server' field in /usr/etc/cobbler/settings.yaml must be set to something other than localhost, "
        ...
        if self.settings.next_server_v4 == "127.0.0.1":
            status.append("For PXE to be functional, the 'next_server_v4' field in /usr/etc/cobbler/settings.yaml must be set to "
                          "something other than 127.0.0.1, and should match the IP of the boot server on the PXE "
                          "network.")
        if self.settings.next_server_v6 == "::1":
            status.append("For PXE to be functional, the 'next_server_v6' field in /usr/etc/cobbler/settings.yaml must be set to "
    ...
    def check_for_default_password(self, status):
        ...
        if default_pass == "$1$mF86/UHC$WvcIcX2t6crBz2onWxyac.":
            status.append("The default password used by the sample templates for newly installed machines ("
                          "default_password_crypted in /usr/etc/cobbler/settings.yaml) is still set to 'cobbler' and should be "
